...
public class ExecutionCompletionService {
    ...
    public void waitForCompletion(long flowId, Process process, String executionId) {
        ...
        try {
            ...
            if (!finished) {
                ...
                saveHistory(flowId, FlowStatus.failed, startedAt, finishedAt, durationMs,
                        ...
                        "Process timeout", List.of(ErrorType.PROCESS_TIMEOUT.value()));
                return;
            }
            ...
            if (processManager.wasStopRequested(flowId)) {
                saveHistory(flowId, FlowStatus.failed, startedAt, finishedAt, durationMs,
                        ...
                        "Manual stop", List.of(ErrorType.MANUAL_STOP.value()));
                return;
            }
            ...
            if (process.exitValue() == 0) {
                ...
                if (result == null) {
                    saveHistory(flowId, FlowStatus.failed, startedAt, finishedAt, durationMs,
                            ...
                            "No JSON payload in stdout", List.of(ErrorType.UNKNOWN.value()));
                    return;
                }
                ...
                List<String> errorTypes = List.of(ErrorType.classify(errorMessage).value());

                saveHistory(flowId, FlowStatus.failed, startedAt, finishedAt, durationMs,
                        stdout != null ? stdout : stderr, payload, errorMessage, errorTypes);
            }
