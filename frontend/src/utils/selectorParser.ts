 ...
 */
function parseHtmlString(html: string): Element | null {
  try {
 ...
 */
function generateSelectorFromElement(el: Element): { selector: string; confidence: 'high' | 'medium' | 'low'; reason: string } {
 ...
 */
function isHtmlElement(input: string): boolean {
 ...
 */
export function parseToSelector(input: string): ParseResult | null {
 ...
 */
export function parseToSelectorSimple(input: string): string | null {
 ...
 */
export function validateSelector(selector: string): { valid: boolean; warning?: string } {
  if (!selector) return { valid: false, warning: '选择器不能为空' };
 ...
 */
export function getSelectorSuggestions(selector: string): string[] {
