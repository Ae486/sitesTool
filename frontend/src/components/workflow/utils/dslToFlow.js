 ...
 */
function convertStepsToFlow(steps, startX, startY, prevNodeId, sourceHandle, depth = 0) {
    const nodes = [];
    const edges = [];
    ...
    for (let i = 0; i < steps.length; i++) {
        ...
        if (isContainer && depth < 4) {
            ...
            if (step.children && step.children.length > 0) {
                ...
                const childResult = convertStepsToFlow(step.children, startX + 250, // Offset to the right
                currentY, nodeId, childHandle, depth + 1);
            ...
            if (step.type === "if_else" && step.else_children && step.else_children.length > 0) {
                const elseResult = convertStepsToFlow(step.else_children, startX + 250, currentY + 200, // Offset down for else branch
                nodeId, "else", depth + 1);
 ...
 */
export function dslToFlow(dsl, options = { applyLayout: true }) {
    nodeIdCounter = 0;
 ...
 */
export function relayoutFlow(nodes, edges) {
