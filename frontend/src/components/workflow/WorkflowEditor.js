...
import { ReactFlow, Background, Controls, MiniMap, Panel, useNodesState, useEdgesState, addEdge, BackgroundVariant, useReactFlow, ReactFlowProvider, } from "@xyflow/react";
import "@xyflow/react/dist/style.css";
import { Button, Space, Tooltip, Segmented, message, Dropdown } from "antd";
import { ReloadOutlined, AppstoreOutlined, UnorderedListOutlined, DeleteOutlined, CopyOutlined, UndoOutlined, RedoOutlined, } from "@ant-design/icons";
...
function WorkflowEditorInner({ value, onChange, onModeChange, siteUrl, readOnly = false, }) {
    ...
    const [selectedNodeIds, setSelectedNodeIds] = useState([]); // Multi-selection
    ...
    useEffect(() => {
        if (hasInitialized.current)
            return; // Already initialized, ignore value changes
    ...
    const startPlacingNode = useCallback((stepType) => {
    ...
    const addNodeAtPosition = useCallback((stepType, position) => {
    ...
    const pasteNode = useCallback(() => {
        if (!clipboard || clipboard.nodes.length === 0 || !reactFlowWrapper.current)
            return;
    ...
    return (_jsxs("div", { style: { display: "flex", height: "100%", minHeight: 500, position: "relative" }, children: [!readOnly && (_jsx("div", { style: {
                    position: "absolute",
                    left: 0,
                    top: 0,
                    bottom: 0,
                    ...
                    flexDirection: "column",
                        ...
                        overflowY: "auto",
                        overflowX: "hidden",
                    }, children: _jsx(NodePalette, { onAddNode: startPlacingNode }) }) })), _jsx("div", { ref: reactFlowWrapper, style: {
                    flex: 1,
                    position: "relative",
                    marginLeft: readOnly ? 0 : 200,
                }, onMouseMove: handleMouseMove, onMouseEnter: () => setIsMouseInCanvas(true), onMouseLeave: () => setIsMouseInCanvas(false), children: _jsxs(ReactFlow, { nodes: nodes, edges: edges, onNodesChange: handleNodesChange, onEdgesChange: onEdgesChange, onConnect: onConnect, onPaneClick: onPaneClick, onDragOver: onDragOver, onDrop: onDrop, onSelectionChange: onSelectionChange, onNodeContextMenu: onNodeContextMenu, onEdgeContextMenu: onEdgeContextMenu, onPaneContextMenu: onPaneContextMenu, nodeTypes: nodeTypes, edgeTypes: edgeTypes, defaultEdgeOptions: defaultEdgeOptions, isValidConnection: isValidConnection, fitView: true, fitViewOptions: { padding: 0.2 }, attributionPosition: "bottom-left", nodesDraggable: !readOnly, nodesConnectable: !readOnly, elementsSelectable: !readOnly, selectionOnDrag: false, panOnDrag: true, selectionKeyCode: "Shift", deleteKeyCode: null, children: [_jsx(Background, { variant: BackgroundVariant.Dots, gap: 20, size: 1, color: "#e0e0e0" }), _jsx(Controls, { showInteractive: false }), _jsx(MiniMap, { nodeStrokeWidth: 3, style: { height: 100, width: 150 }, maskColor: "rgba(0, 0, 0, 0.1)", position: "top-left" }), _jsx(Panel, { position: "top-right", children: _jsxs(Space, { style: {
                                    background: "#fff",
                                    padding: "6px 12px",
                                    borderRadius: 8,
                                    boxShadow: "0 2px 8px rgba(0,0,0,0.1)",
                                }, children: [isMultiSelect && !readOnly && (_jsxs(_Fragment, { children: [_jsxs("span", { style: { fontSize: 12, color: "#666" }, children: ["\u5DF2\u9009 ", selectedNodeIds.length, " \u4E2A"] }), _jsx(Tooltip, { title: "\u5220\u9664\u9009\u4E2D (Delete)", children: _jsx(Button, { icon: _jsx(DeleteOutlined, {}), size: "small", danger: true, onClick: deleteSelectedNodes }) })] })), !isMultiSelect && selectedNode && !readOnly && (_jsxs(_Fragment, { children: [_jsx(Tooltip, { title: "\u590D\u5236 (Ctrl+C)", children: _jsx(Button, { icon: _jsx(CopyOutlined, {}), size: "small", onClick: copyNode }) }), _jsx(Tooltip, { title: "\u5220\u9664 (Delete)", children: _jsx(Button, { icon: _jsx(DeleteOutlined, {}), size: "small", danger: true, onClick: () => deleteNode(selectedNodeId) }) })] })), !readOnly && (_jsxs(_Fragment, { children: [_jsx(Tooltip, { title: "\u64A4\u9500 (Ctrl+Z)", children: _jsx(Button, { icon: _jsx(UndoOutlined, {}), size: "small", onClick: undo, disabled: !canUndo }) }), _jsx(Tooltip, { title: "\u91CD\u505A (Ctrl+Y)", children: _jsx(Button, { icon: _jsx(RedoOutlined, {}), size: "small", onClick: redo, disabled: !canRedo }) })] })), _jsx(Tooltip, { title: "\u81EA\u52A8\u5E03\u5C40", children: _jsx(Button, { icon: _jsx(ReloadOutlined, {}), size: "small", onClick: handleRelayout }) }), onModeChange && (_jsx(Segmented, { size: "small", options: [
                                            { value: "workflow", icon: _jsx(AppstoreOutlined, {}) },
                                            { value: "list", icon: _jsx(UnorderedListOutlined, {}) },
                                        ], value: "workflow", onChange: (v) => onModeChange(v) }))] }) }), nodes.length <= 1 && (_jsx(Panel, { position: "top-center", style: { marginTop: 100 }, children: _jsx("div", { style: {
                                    background: "#fff",
                                    padding: "12px 20px",
                                    borderRadius: 8,
                                    boxShadow: "0 2px 8px rgba(0,0,0,0.1)",
                                    textAlign: "center",
                                }, children: _jsx("span", { style: { color: "#8c8c8c", fontSize: 13 }, children: "\u4ECE\u5DE6\u4FA7\u70B9\u51FB\u6216\u62D6\u62FD\u8282\u70B9\u5230\u753B\u5E03\uFF0C\u7136\u540E\u8FDE\u63A5\u5230\u5F00\u59CB\u8282\u70B9" }) }) }))] }) }), !isMultiSelect && selectedNode && !readOnly && selectedNode.id !== START_NODE_ID && (_jsx("div", { style: {
                    width: 280,
                    borderLeft: "1px solid #f0f0f0",
                    overflow: "auto",
                    background: "#fff",
                }, children: _jsx(NodeProperties, { node: selectedNode, onUpdate: (data) => updateNodeData(selectedNode.id, data), onDelete: () => deleteNode(selectedNode.id) }) })), pendingNodeType && isMouseInCanvas && (_jsx(GhostNode, { stepType: pendingNodeType, x: ghostPosition.x, y: ghostPosition.y })), contextMenu && (_jsx(Dropdown, { menu: { items: contextMenuItems }, open: true, onOpenChange: (open) => !open && setContextMenu(null), children: _jsx("div", { style: {
                        position: "fixed",
                        left: contextMenu.x,
                        top: contextMenu.y,
                        width: 1,
...
export default function WorkflowEditor(props) {
    return (_jsx(ReactFlowProvider, { children: _jsx(WorkflowEditorInner, { ...props }) }));
}
